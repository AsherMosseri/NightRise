...
import {
  ...
  nightCompletionBonus, checkBadges, COMBO_MAX, COMBO_WINDOW_MS,
} from '../js/game.js';
...
test('a chain only continues inside the combo window', () => {
  const night = { lastDoneAt: 1000, combo: 1.25 };
  assert.equal(chainLengthFor(night, 1000 + COMBO_WINDOW_MS - 1), 3);
  assert.equal(chainLengthFor(night, 1000 + COMBO_WINDOW_MS + 1), 1);
  assert.equal(chainLengthFor({ lastDoneAt: 0, combo: 1 }, 5000), 1);
});
...
test('back-to-back completions build the combo', () => {
  const state = createInitialState();
  const [a, b, c] = Object.values(state.template.tasks);
  const first = applyTaskCompletion(state, a, 1_000_000);
  const second = applyTaskCompletion(state, b, 1_010_000);
  const third = applyTaskCompletion(state, c, 1_020_000);
...
test('a slow night never builds a combo', () => {
  const state = createInitialState();
  const [a, b] = Object.values(state.template.tasks);
  applyTaskCompletion(state, a, 1_000_000);
  const second = applyTaskCompletion(state, b, 1_000_000 + COMBO_WINDOW_MS * 3);
